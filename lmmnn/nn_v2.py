...
#     out_hidden = add_layers_sequential(X_input, n_neurons, dropout, activation, X_train[x_cols].shape[1])
    ...
    if mode in ['spatial', 'spatial_embedded']:
        ...
        sig2b_spatial_ests = []
    y_pred_tr = model.predict([X_train[x_cols], y_train] + X_train_z_cols, verbose=verbose).reshape(X_train.shape[0])
    ...
    if mode in ['intercepts', 'glmm', 'spatial', 'spatial_and_categoricals']:
        if Z_non_linear or len(qs) > 1 or mode == 'spatial_and_categoricals':
            ...
            y_pred = model.predict([X_test[x_cols], dummy_y_test] + X_test_z_cols, verbose=verbose).reshape(
                X_test.shape[0]) + b_hat[X_test['z0']]
            y_pred_fe = model.predict([X_test[x_cols], dummy_y_test] + X_test_z_cols, verbose=verbose).reshape(
                X_test.shape[0])
        if mode == 'glmm':
    ...
    return y_pred_tr, y_pred_fe, b_hat, history, y_pred, (sig2e_est, list(sig2b_ests), list(sig2b_spatial_ests)), list(rho_ests), list(weibull_ests), len(history.history['loss'])
